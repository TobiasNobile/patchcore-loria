"""Superpose la heatmap d'anomalie PatchCore sur des frames test SOHAS.

Charge une banque construite par bin/sohas/fit/memory_bank.py — pas de fit ni de
coreset, c'est la moitié rapide de PatchCore. Le prétraitement et le seed sont
relus du fit_config.json de la banque. C'est l'aperçu hors-ligne de ce que
bin/live_web.py affichera en direct sur la webcam.

    python bin/sohas/infer/heatmap.py                      # une frame (défaut)
    python bin/sohas/infer/heatmap.py --image_index 50     # une autre frame
    python bin/sohas/infer/heatmap.py --n_per_class 30     # 30 weapon + 30 good
"""
import logging
import os
import platform
...
# Mode image seule : index dans le split TEST équilibré. Ignoré si --n_per_class > 0.
IMAGE_INDEX_DEFAULT = 0

# Sorties ici. En mode image seule, {idx} est remplacé. En mode --n_per_class,
# un sous-dossier par banque est créé, fichiers <weapon|good>_idx<N>.png.
OUTPUT_PATH = "results/sohas/heatmaps/overlay_idx{idx}.png"
...
@click.option(
    ...
    show_default=True,
    help="Mode image seule : index dans le split TEST équilibré de SOHAS.",
)
@click.option(
    "--n_per_class",
    type=int,
    default=0,
    show_default=True,
    help="Si >0, ignore --image_index : tire n frames weapon et n good au hasard "
    "(effectifs égaux) et superpose chacune.",
)
def main(image_index, n_per_class):
    ...
    # Labels connus sans inférence : le tirage good/weapon se fait en amont.
    if n_per_class > 0:
        ...
        if n < n_per_class:
            LOGGER.warning(
                "n_per_class=%d requested but only %d good / %d weapon available "
                "=> using n=%d per class.",
        ...
        out_dir = os.path.join(
            os.path.dirname(OUTPUT_PATH) or "results/sohas/heatmaps",
    ...
    LOGGER.info("Scored %d frame(s), %.1f ms/image.", len(selected), ms)

"""Superpose la heatmap d'anomalie PatchCore sur des images test COCO (personne
avec/sans couteau).

Charge une banque construite par bin/coco/fit/memory_bank.py — pas de fit. Le
prétraitement et le seed sont relus du fit_config.json.
...
OUTPUT_PATH = os.environ.get("HEATMAP_OUTPUT_PATH", "results/coco/heatmaps/overlay_idx{idx}.png")
...
def _save_overlay(image, heatmap, score, anomaly, ms, out_path):
    ...
    plt.imshow(image)
    plt.imshow(heatmap, cmap="jet", alpha=HEATMAP_ALPHA, vmin=HEATMAP_VMIN, vmax=HEATMAP_VMAX)
...
@click.command()
@click.option("--image_index", type=int, default=IMAGE_INDEX_DEFAULT, show_default=True,
              help="Mode image seule : index dans le split TEST équilibré.")
@click.option("--n_per_class", type=int, default=0, show_default=True,
              help="Si >0, ignore --image_index : n images knife + n good au hasard, "
                   "chacune superposée (n_per_class=15 -> 30 heatmaps).")
def main(image_index, n_per_class):
    ...
    if n_per_class > 0:
        ...
        if n < n_per_class:
            LOGGER.warning(
                "n_per_class=%d demandé mais %d good / %d knife dispo => n=%d.",
        ...
        out_dir = os.path.join(
            os.path.dirname(OUTPUT_PATH) or "results/coco/heatmaps",
    ...
    for pos, idx in enumerate(selected):
        ...
        if out_dir is not None:
            out_path = os.path.join(out_dir, "{}_idx{}.png".format(sample["anomaly"], idx))
        else:
            out_path = OUTPUT_PATH.format(idx=idx)
        _save_overlay(image, np.array(segmentations[pos]), scores[pos], sample["anomaly"], ms, out_path)
